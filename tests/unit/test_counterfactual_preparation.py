...
def language_candidate_set(ordinal: int) -> CandidateSet:
    return CandidateSet(
        candidate_set_id=f"language-candidates-{ordinal}",
...
def language_trial(ordinal: int) -> LanguageBenchmarkTrial:
    ...
    return LanguageBenchmarkTrial(
        trial_id=f"language-trial-{ordinal}",
        profile_id="synthetic-concise",
        message_id="message-with-two-spans",
        span_index=ordinal,
        ...
        candidate_set=language_candidate_set(ordinal),
...
def test_preparation_rejects_ineligible_protocols_and_invalid_recipes(
    ...
    with pytest.raises(ValueError, match="cannot limit complete messages"):
        CounterfactualPreparationSpec(
