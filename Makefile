...
candidate-generation-step4:
	uv run python scripts/run_candidate_generation_step4.py $(CANDIDATE_GENERATION_STEP4_ARGS)
...
manuscript:
	uv run python scripts/build_manuscript.py $(MANUSCRIPT_ARGS)

manuscript-latex-sync:
	uv run python scripts/build_manuscript.py --sync-latex-source $(MANUSCRIPT_ARGS)

publication-release:
	uv run python scripts/build_publication_release.py $(PUBLICATION_RELEASE_ARGS)

publication-release-check: public-release-check
	uv run python scripts/check_publication_release.py $(PUBLICATION_RELEASE_CHECK_ARGS)

journal-submission:
	@test -n "$(JOURNAL)" || (echo "JOURNAL=rbet or JOURNAL=neuroinformatics is required" >&2; exit 2)
	uv run python scripts/build_journal_submission.py --journal $(JOURNAL) $(JOURNAL_SUBMISSION_ARGS)

journal-submission-check:
	@test -n "$(JOURNAL)" || (echo "JOURNAL=rbet or JOURNAL=neuroinformatics is required" >&2; exit 2)
	uv run python scripts/check_journal_submission.py \
		--output artifacts/submission/$(JOURNAL)-v1 \
		$(JOURNAL_SUBMISSION_CHECK_ARGS)
